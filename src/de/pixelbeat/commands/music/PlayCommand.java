...
public class PlayCommand implements ServerCommand{
	...
	public void performCommand(Member m, TextChannel channel, Message message, Guild guild) {
		...
		if(args.length > 1) {
			...
			MusicUtil.sendEmbledError(guild.getIdLong(), mf.format(guild.getIdLong(), "feedback.info.command-usage",Utils.getGuildPrefix(guild.getIdLong())+"play <url/search query>"));
		}
	}
}
