		...
		if(args.length == 1) {
			...
			try {
				switch(SubCommand.valueOf(args[1].toUpperCase())) {
					...
					case DJROLES:
						try {
							Long value = Long.decode(args[2]);
							if(guildentity.addDJRoleID(value)) {
								channel.sendMessage("succes add "+ value).queue();	
							}else if(guildentity.removeDJRoleID(value)) {
								channel.sendMessage("succes remove "+ value).queue();	
							}
						}catch (NumberFormatException e) {
							message.getMentionedRoles().forEach((role) -> {
								if (guildentity.addDJRoleID(role.getIdLong())) {
									channel.sendMessage("succes add " + role.getIdLong()).queue();
								} else if (guildentity.removeDJRoleID(role.getIdLong())) {
									channel.sendMessage("succes remove " + role.getIdLong()).queue();
								}
							});
						}
