		...
		if((state = guild.getSelfMember().getVoiceState()) != null && (vc = state.getChannel()) != null) {
			...
			if(queue.isPlayingTrack()) {
				if(args.length <= 1) {
					...
					track.setPosition(player.getPlayingTrack().getPosition()+10000l);
				}else {
					String subTime = "";
					...
					AudioTrack track = player.getPlayingTrack();
					track.setPosition(player.getPlayingTrack().getPosition()+Utils.decodeTimeMillisFromString(subTime));
				}
			}else 
